...
class ProductController extends Controller {
    ...
    public function delete($id) {
        // Lấy thông tin sản phẩm trước khi xóa (để xóa ảnh nếu cần).
        ...
        if ($product) {
            ...
            if ($this->productModel->deleteProduct($id)) {
                ...
                // (Tùy chọn) Xóa file ảnh vật lý để tiết kiệm dung lượng server.
                // Nếu muốn giữ ảnh để khôi phục thì bỏ đoạn này đi.
                if (!empty($product->image)) {
                    $imagePath = '../public/uploads/' . $product->image;
                    // file_exists(): Kiểm tra file có tồn tại không.
                    if (file_exists($imagePath)) {
                        // unlink(): Hàm xóa file khỏi ổ cứng.
                        unlink($imagePath);
                    }
                }
