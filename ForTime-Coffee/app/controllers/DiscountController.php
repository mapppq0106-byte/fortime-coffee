<?php
//Vai trò: Controller này quản lý toàn bộ chức năng liên quan đến Mã giảm giá (Discount Codes).

//Nhiệm vụ chính:

//Hiển thị: Danh sách các mã giảm giá (bao gồm cả mã đã xóa mềm).

//Thêm mới: Tạo mã giảm giá với các điều kiện (giá trị giảm, đơn tối thiểu).

//Cập nhật: Sửa thông tin mã giảm giá hiện có.

//Xóa & Khôi phục: Xóa mềm mã (đưa vào thùng rác) và khôi phục lại khi cần.

//Bật/Tắt: Thay đổi trạng thái hoạt động nhanh chóng.
// Định nghĩa class DiscountController.
// 'class': Tạo một lớp đối tượng.
// 'extends Controller': Kế thừa từ class cha Controller để sử dụng các tính năng chung (model, view, redirect...).
class DiscountController extends Controller {
    ...
    public function add() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            // isset(): Kiểm tra checkbox 'Kích hoạt ngay' có được tích không.
            // Nếu có -> 1 (Bật), Không -> 0 (Tắt).
    ...
    // Nhận ID và trạng thái mới (0 hoặc 1) từ URL.
    public function toggle($id, $status) {
        // Gọi Model cập nhật cột is_active.
    ...
    public function restore($id) {
        ...
        // Hàm này sẽ set is_deleted = 0 (Hồi sinh) và is_active = 0 (Tắt để an toàn).
