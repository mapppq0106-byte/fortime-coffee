...
class UserModel {
    ...
    public function login($username, $password) {
        ...
        if (password_verify($password, $row->password_hash)) {
            // Kiểm tra thêm: Tài khoản có đang hoạt động (is_active = 1) không?
            if ($row->is_active == 0) {
    ...
    public function deleteUser($id) {
        // [QUERY] Đánh dấu xóa (is_deleted=1) VÀ Tắt hoạt động (is_active=0)
        // Để user này không thể đăng nhập được nữa ngay lập tức.
        $sql = "UPDATE users SET is_deleted = 1, is_active = 0 WHERE user_id = :id";
    ...
    public function restoreUser($id) {
        // [QUERY] Bỏ đánh dấu xóa (is_deleted=0) VÀ Bật lại hoạt động (is_active=1)
        $sql = "UPDATE users SET is_deleted = 0, is_active = 1 WHERE user_id = :id";
